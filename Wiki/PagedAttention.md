---
date-created: 2026-04-14 15:30
date-updated: 2026-04-15 00:30
tags:
  - 🤖
---
...
- [[KV Cache]] — 为什么要缓存 K/V，以及每一步 decode 需要读取哪些内容
- [[LLM Inference Optimization#Prefill vs Decode]] — 推理两阶段的特性差异
- 【非必需】操作系统中的**虚拟内存 / 分页**机制——PagedAttention 的命名直接来源于此

> [!abstract] PagedAttention 是什么
> **PagedAttention 将 KV Cache 从"连续大块"改为"固定大小的小块 + 块表映射"**，借鉴操作系统虚拟内存的思路管理显存，使内存碎片率从 60–80% 降至 <4%，吞吐量提升 2–3 倍。它是 vLLM 的核心技术[^1]。

## 问题：朴素 KV Cache 的内存浪费

### KV Cache 到底有多大？

以 LLaMA-2-13B、FP16 为例：
...
vLLM 之前的主流做法是：**请求到达后即按 `max_tokens` 预分配一整段连续显存**。方案实现简单，但显存浪费严重，具体分为两类。

#### 内部碎片（Internal Fragmentation）

按最大可能长度一次性预留显存，但实际生成长度通常远小于此值。例如：
...
由于不同请求的预分配大小不一，它们完成并释放后会在显存中留下大小各异的空隙：
...
操作系统面临的内存管理问题与 KV Cache 的问题在结构上完全相同：
...
设 prompt 长度为 $N$、块大小为 $B$，则需要 $\lceil N/B \rceil$ 个物理块。分配流程：

1. 从空闲块池取 $\lceil N/B \rceil$ 个物理块；
2. 将这些物理块 ID 依次写入请求的块表；
3. 执行 prefill attention，并按块表将 K/V 写入对应物理块。
...
每生成一个 token：若当前最后一个逻辑块尚未写满，则写入下一个空槽位；若已写满，则从空闲池取一个新块挂接到块表末尾。
...
## PagedAttention Kernel：分散的块怎么算 attention？
...
softmax 的作用：**将任意大小的一组数转成加起来为 1 的权重**。
...
$e^x$ 增长极快，$x=100$ 时结果已超出浮点数可表示范围。所有主流 softmax 实现都先减最大值再取指数：

$$
\text{softmax}(s)_t = \frac{e^{s_t - m}}{\sum_u e^{s_u - m}}, \quad m = \max_u s_u
$$

分子分母各多一个 $e^{-m}$ 因子，相互约去——数学上等价，但现在 $e^{s_t - m} \le 1$，永不溢出。

**代价**：必须先扫完所有分数才能拿到最大值 $m$。

### PagedAttention 下的新问题

标准做法"先扫一遍找 $m$、再扫一遍算 softmax"在 PagedAttention 下代价翻倍：
...
核心思路：**边遍历边累加，遇到更大的最大值就对之前的累积做一次修正**。
...
一行推导说明为何"乘 $e^{m_\text{old} - m_\text{new}}$"即可把旧累积换到新基准：
...
### 分块遍历 + Online Softmax = PagedAttention Kernel

标准 attention 把 $qK^\top$ 当作一次大 GEMM；PagedAttention 因为 K/V 分散，必须拆成逐块处理，每块结束后用 online softmax 合并：
...
**写时复制**：
持有 `ref_count > 1` 的共享块的一方需要写入新 token 时：
...
更重要的是，它确立了**"推理服务 = 操作系统式显存管理 + 动态调度"** 的工程范式，为后续实践奠定基础。
