# AI-Infra Vault — Codex 工作规则

## 角色边界

- **用户职责**：sourcing（找资料、Web Clipper、读 paper、读代码）、提问、review、决定方向
- **Codex 职责**：消化原料、写 wiki 条目（overview + atomic）、维护互链、保持风格一致、维护 frontmatter
- **`Sources/` 是只读区**：Codex 不修改 `Sources/Clippings/` 和 `Sources/Papers/` 下的任何文件
- **`Wiki/` 是 Codex 主写区**：用户做 review，必要时小修

## 不确定即询问（强约束）

遇到不能 100% 确认的内容**不写到页里**——转而询问用户。零半信半疑产出。

"不确定"涵盖四类，任一类不确定都必须停下问：
- **事实正确性**不确定（数字、版本号、API、commit hash 等）
- **用户是否学过**不确定 → 不要替用户决定他懂了
- **深度是否超出用户当前掌握**不确定 → 不要写超纲细节
- **是否需要新拆 / 新建页**不确定 → 不要替用户决定知识结构

## 保守扩展原则（强约束）

**Wiki 是用户已消化的内容快照，不是源材料 digest。**

落笔到 Wiki 的每一条事实 / 段落，前提是用户已经在对话中表达过"我读过 / 我学过 / 我理解了"。仅出现在 `Sources/` 但用户没说学过的内容**不写入 Wiki**。

推论：
- digest 一篇 paper / clipping ≠ 把内容搬进 Wiki。digest 只产出"对话内的摘要回复"，wiki 落笔需用户二次确认"这部分我学完了，写进去吧"。
- 不在用户没读过的细节上"补完整图景"。**宁可页面残缺，也不替用户补脑。**
- 扩展默认保守而非激进。拿不准就少写、留 stub、问用户。

## 单次任务的扩展上限（强约束）

一次用户请求内，Codex 最多做以下其一：
- 新建 **1** 个 wiki 页（含 Lead + Related，其余按"Atomic 页"最小形态办）
- 给现有页加 **1** 个章节
- 在 ≤ 3 个已有页里做局部润色 / 修错 / 互链补充

**禁止单次任务里 "queue N 个 atomic pages"、"spin off 多个 stub"、"extend stack with 多个 atomic"。** 超出上限必须先停下报告"我建议下一步做 X, Y, Z，先做哪个？"由用户挑一个。

## 禁止主动 queue 后续页（强约束）

Codex 不在 `TODOs.md` 的"建议补充"节里主动塞条目。该节只在以下两种情况由 Codex 写入：
1. 用户明说"queue 一下 X" / "把 Y 加到建议补充里"
2. 用户在对话中明确认可了某条具体后续工作

如果发现 "建议补充" 里已有 Codex 之前主动加的条目，下次接触时跟用户确认是否保留，不要默认沿用。
...
---
```
不加 `tags`、不加 `status`。`updated` 由 Linter 自动维护；Codex 编辑后如 Linter 未触发，手动改为今日日期。
...
- Lead（定义）必有
- Related 节必有
- **中间章节（Mechanism 等）按需，可留空**。最小可接受形态 = Lead + Related，Mechanism 等用户没学到就别替他写
- 不加页内 WIP 标记（见 D.16）
...
- message 形式：`<type>: <change>`
  ...
  - `chore: update AGENTS.md style rules`
- 不强制 commit cron；历史清洁度比覆盖率重要

## index.md 同步（强约束）

`Wiki/index.md` 是按主题分组的全 wiki 目录，属 ingest 流程一部分，必须同步维护——任何新增 / 删除 / 重命名 wiki 页都要回到这里改。

**新建 wiki 页时**：
1. 在 `Wiki/index.md` 对应主题分组下加一行：`- [[X]] —— 一句话摘要`
2. 一句话摘要从该页 Lead 段提炼，控制在 ~80 字以内
3. 若新页不属于任何现有分组，**先停下询问用户**是否新增分组——不擅自创建

**删除 wiki 页时**：同步删除 `index.md` 里的对应条目。

**重命名 wiki 页时**：依下方"反向链接更新规则"处理，grep 自然会扫到 `index.md`。

**修改 Lead 段时**：仅当 Lead 段语义实质变化（核心定义改了）才同步更新摘要；纯措辞润色不必动。
